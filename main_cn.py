...
class _SilenceThinkingWarning(logging.Filter):
    """过滤 agentscope 关于 thinking 块的无关警告"""

    def filter(self, record: logging.LogRecord) -> bool:
        return "Unsupported block type" not in record.getMessage()


logging.getLogger("as").addFilter(_SilenceThinkingWarning())
...
class GameReActAgent(ReActAgent):
    """游戏专用智能体：隐藏思考过程、优雅处理中断"""

    async def print(self, msg: Msg, last: bool = True) -> None:
        """只打印文本发言，隐藏 thinking 思考过程（流式增量打印）"""
        ...
        printed = self._stream_prefix.get(msg.id, "")
        target = f"{msg.name}: {texts}" if texts else printed
        if len(target) > len(printed):
            print(target[len(printed):], end="")
            self._stream_prefix[msg.id] = target
        if last and msg.id in self._stream_prefix:
            print()
            self._stream_prefix.pop(msg.id, None)

    async def handle_interrupt(self, *args, **kwargs) -> Msg:
        """回复被中断时优雅降级，避免因多余参数崩溃"""
        return Msg(
            name=self.name,
            content="（本轮发言被中断，跳过）",
            role="assistant",
